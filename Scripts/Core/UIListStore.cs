...
namespace Tuick.Core
{
	public class UIListStore
	{
		...
		private void Initialize()
		{
			...
			if (localUxmlListData == null)
			{
				...
				try
				{
					...
					AssetDatabase.ImportAsset(uxmlListPath,
				...
				catch (System.Exception e)
				{
					...
					Debug.LogErrorFormat("UIListStore: Error creating UXMLList asset at '{0}': {1}\nStackTrace: {2}",
						uxmlListPath, e.Message, e.StackTrace);
				}
			}
			...
			if (localUssListData == null)
			{
				...
				try
				{
					...
					AssetDatabase.ImportAsset(ussListPath,
						ImportAssetOptions.ForceUpdate | ImportAssetOptions.ForceSynchronousImport);
		...
		public void Refresh()
		{
			...
			{
				Debug.LogError(
					"UIListStore: UXMLList or USSList object not found in Resources folder. UI System may not function correctly.");
			}
		}
	}
}